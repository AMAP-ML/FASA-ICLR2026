...
def local_heavy_hitter_mask(attn_weights, token_budget, chunk_size):
    ...
    _, topk = chunk_attn_weights.topk(
        k=min(max(3, int(token_budget // chunk_size)), chunk_attn_weights.size(-1)), dim=-1
    )
...
def forward(
    ...
    **kwargs,
    ...
    # cos, sin = self.rotary_emb(value_states, position_ids.to(value_states.device)) #yf before
    rotary_seq_len = max(kv_seq_len, position_ids[:, -1].max().item()) + 1 #yf later
    cos, sin = self.rotary_emb(value_states, seq_len=rotary_seq_len) #yf later
    query_states, key_states = apply_rotary_pos_emb(#yf later
    ...
    # print(f"================={key_states.shape,self.num_key_value_heads}====================")
...
def enable_quest_attention_eval(model, args):
    for name, module in reversed(model._modules.items()):
        ...
        if isinstance(module, (LlamaAttention, MistralAttention,Qwen2Attention)):
            ...
            model._modules[name].token_budget = args.budget
