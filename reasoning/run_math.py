...
def evaluate_reasoning_df(model,tokenizer,dataset_name,eval_batch_size=1,limit=1):
    prompts, test_data = load_prompts_and_data(dataset_name,limit)
...
def main(args):
    ...
    fout = open(f"{args.save_path}", "a")
    prompts_subsets = [prompts[i::world_size] for i in range(world_size)]
    test_data_subsets = [test_data[i::world_size] for i in range(world_size)]
    processes = []
    mp.set_start_method('spawn', force=True)
    for rank in range(world_size):
        p = mp.Process(target=evaluate_reasoning_worker, \
            args=(args,args.model_name,prompts_subsets[rank],test_data_subsets[rank],args.save_path,args.dataset_name,rank,args.eval_batch_size))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()
...
def parse_arguments():
    ...
    parser.add_argument("--output_dir",type=str,default="reasoning")
...
if __name__ == "__main__":
    ...
    os.makedirs(f"{args.output_dir}/{args.dataset_name}_{args.cal_dataset}", exist_ok=True)
    if args.method == "base":
        save_path = f"{args.output_dir}/{args.dataset_name}/{args.model_name}_{args.dataset_name}_{args.method}_{args.seed}.jsonl"
    elif args.method in [ "oracle","snapkv"]:
        save_path = f"{args.output_dir}/{args.dataset_name}/{args.model_name}_{args.dataset_name}_{args.method}_{args.budget}_{args.seed}.jsonl"
    elif args.method == "dynamic_frequency":
        save_path = f"{args.output_dir}/{args.dataset_name}_{args.cal_dataset}/{args.model_name}_{args.dataset_name}_{args.method}_{args.budget}_{args.min_k}_{args.max_k}_{args.seed}.jsonl"
    elif args.method == "quest":
        save_path = f"{args.output_dir}/{args.dataset_name}/{args.model_name}_{args.dataset_name}_{args.method}_{args.budget}_{args.seed}.jsonl"
