...
def main(model_name,task_type,prefilling_len,decoding_len,dataset_name):
    ...
    sparsity_list=[128,256,512,1024,2048,4096]
    ...
    if task_type == "longbench":
        for dataset in [dataset_name]:  
            total_tokens=0
            ranks_file_name = os.path.join(ranks_dir,f"{dataset}_{args.evaluate_num}.pkl")
            if dataset2maxlen[dataset] >=512:
                ...
                eval_longbench(model, tokenizer, model_name, datasets=[dataset],output_dir=output_dir,evaluate_num=args.evaluate_num)
            with open(ranks_file_name,"wb") as f:
                dill.dump(res_dict,f)
